function[] = plot_intensity_spd(group_speed)

if ~isfield(group_speed,'mean_intensity')
    error('Necessary fields missing.')
else
    nspd = length(group_speed);
    
    ncells = size(group_speed(1).mean_intensity,2);
    ...
    for ispd = 1:nspd
        for icell = 1:ncells
            plot(ispd,group_speed(ispd).mean_intensity_spd_mean(icell),'o', 'MarkerEdgeColor',cell_col(icell,:));
            hold on;
%             errorbar(ispd,group_speed(ispd).mean_intensity_spd_mean(icell),group_speed(ispd).mean_intensity_spd_sem(icell),'Color',cell_col(icell,:));
        ...
        plot(ispd,mean(group_speed(ispd).mean_intensity_spd_mean),'ko', 'MarkerFaceColor','k');
        errorbar(ispd,mean(group_speed(ispd).mean_intensity_spd_mean),std(group_speed(ispd).mean_intensity_spd_mean,[],2)/sqrt(ncells),'Color','k');
    ...
    for ispd = 1:nspd
        for icell = 1:ncells
            plot(ispd,group_speed(ispd).area_intensity_spd_mean(icell),'o', 'MarkerEdgeColor',cell_col(icell,:));
            hold on;
        end
        
        plot(ispd,mean(group_speed(ispd).area_intensity_spd_mean),'ko', 'MarkerFaceColor','k');
        errorbar(ispd,mean(group_speed(ispd).area_intensity_spd_mean),std(group_speed(ispd).area_intensity_spd_mean,[],2)/sqrt(ncells),'Color','k');
