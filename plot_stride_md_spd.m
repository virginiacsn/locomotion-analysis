    ...
    if plt_cell
        ...
        for ipaw = paw_order
            ...
            for ispd = 1:length(group_speed)
                ...
                if ispd == 1
                    [hax,li1,li2] = plotyy(1:10,group_speed(ispd).stride_bins_mean_spd_mean{ipaw}(:,1),1:10,group_speed(ispd).img_bins_mean_spd_mean{ipaw});
                    ...
                    li1(end+1) = plot(hax(1),1:10,group_speed(ispd).stride_bins_mean_spd_mean{ipaw}(:,1));
                    li2 = [li2; plot(hax(2),1:10,group_speed(ispd).img_bins_mean_spd_mean{ipaw})];
                ...
                hold(hax(1),'on');
                plot_error_shade(hax(1), 1:10, [], [], group_speed(ispd).stride_bins_mean_spd_mean{ipaw}(:,1),...
            ...
            for i = 1:2
                hax(i).YLim = [round(min(ylimit(i:2:end,1))*0.9) round(max(ylimit(i:2:end,2))*1.1)];
                hax(i).YTick = round(linspace(hax(i).YLim(1), hax(i).YLim(2), 5));
            end
    ...
    if plt_mean_cell
        ...
        for ipaw = paw_order
            ...
            for ispd = 1:length(group_speed)
                ...
                if ispd == 1
                    [hax,li1,li2] = plotyy(1:10,group_speed(ispd).stride_bins_mean_spd_mean{ipaw}(:,1),1:10,group_speed(ispd).img_bins_mean_cell_spd_mean{ipaw});
                    li1.Marker = spd_mrk{ispd};
                    li2.Marker = spd_mrk{ispd};
                    li2.Color = spd_col(ispd,:);
                else
                    li1(end+1) = plot(hax(1),1:10,group_speed(ispd).stride_bins_mean_spd_mean{ipaw}(:,1));
                    li2(end+1) = plot(hax(2),1:10,group_speed(ispd).img_bins_mean_cell_spd_mean{ipaw});
                ...
                hold(hax(2),'on');
                plot_error_shade(hax(2), 1:10, [], [], group_speed(ispd).img_bins_mean_cell_spd_mean{ipaw},...
